...
The cursor tracker uses `GetCursorPos` to poll physical virtual-screen coordinates every 50 ms only while manually enabled. It resets when Tarkov is unavailable, minimized, or the cursor leaves the game window; Tarkov does not need to own foreground focus. After 250 ms within a three-pixel tolerance, it emits one recognition trigger and does not repeat it until the cursor moves or tracking is reset.
